...
- 항공편/혼잡도 API 호출을 CompletableFuture 기반 병렬 처리로 전환하여 데이터 수집 속도를 2배 이상 개선하고, JDBC Batch Insert를 적용하여 수천 건의 데이터를 효율적으로 저장 및 갱신할 수 있도록 하였습니다.<br>
  ( 항공편 데이터는 2천건 미만으로 성능 개선이 크진 않지만, 확장성 관점에서 적용했습니다. )
...
- React 개발 서버 실행 시 발생하던 메모리 부족 문제를 해결하기 위해, Nginx를 통해 빌드된 정적 파일을 서빙하는 방식으로 변경하였습니다.
-   
- Oauth 적용을 위해 도메인을 적용하고 Http 프로토콜을 Https 프로토콜로 변경하였습니다.<br>
