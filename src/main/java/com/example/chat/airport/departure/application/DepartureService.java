package com.example.chat.airport.departure.application;
import com.example.chat.airport.departure.domain.Departure;

import com.example.chat.airport.departure.ui.DepartureResDto;
import com.example.chat.airport.departure.domain.CongestionAlertEvent;
import com.example.chat.airport.departure.domain.DepartureRepository;
...
public class DepartureService {
    ...
    private static final long CONGESTION_THRESHOLD = 8200;
    ...
    private void updateWithCongestionCheck(Departure exists, String date, String timeZone,
        ...
        boolean prevT1Busy = exists.getT1Gates().isCongested(CONGESTION_THRESHOLD);
        boolean prevT2Busy = exists.getT2Gates().isCongested(CONGESTION_THRESHOLD);

        exists.updateDeparture(newT1Gates, newT2Gates);

        // 터미널별로 독립적으로 판단 - 혼잡 상태로 진입한 터미널의 구독자에게만 발송
        if (!prevT1Busy && newT1Gates.isCongested(CONGESTION_THRESHOLD)) {
            publishCongestionAlert(date, timeZone, "T1", newT1Gates, newT2Gates);
        }
        if (!prevT2Busy && newT2Gates.isCongested(CONGESTION_THRESHOLD)) {
